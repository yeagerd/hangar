...
func Register(s *server.MCPServer, mgr Manager, capture PaneCapture, storeUpd StoreUpdater, defaultThresholdMs int64) {
	...
	s.AddTool(mcp.NewTool("workspace_list",
		...
		if waitAny && waitAll {
			return toolError("internal", "wait_any_idle and wait_all_idle are mutually exclusive"), nil
		}
	...
	s.AddTool(mcp.NewTool("workspace_send",
		...
		if !ok {
			return toolError("internal", fmt.Sprintf("rate limited: retry after %d ms", retryAfterMs)), nil
		}
